...
class DescriptorStorage {
    public:
        ...
        virtual std::unordered_map<T, unsigned long> GetAllDescriptors(const std::string& reconstruction_id, const std::string& image_id) = 0;
        virtual void DeleteAllDescriptors(const std::string& image_id) = 0;
        virtual std::set<std::string> GetAllWithCommonDescriptors(const std::string& image_id, int page_size = 0, std::string* page_start = nullptr) = 0;
