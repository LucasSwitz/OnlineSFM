...
public:
    TFIDFRanker(std::shared_ptr<DescriptorStorage<SIFT_Descriptor>> word_storage,
                std::shared_ptr<ImageStorageAdapter> image_storage) : _image_storage(image_storage),
    ...
    {
        {
            PrecisionTimer t("TFIDRanker.Rank.GetAllDescriptors");
            this->_query_word_frequencies = this->_word_storage->GetAllDescriptors(reconstruction_id, query_image_id);
        }
        {
            PrecisionTimer t("TFIDRanker.Rank.GetGlobalDescriptorFrequencies");
            this->_global_word_frequencies = this->_word_storage->GetGlobalDescriptorFrequencies(reconstruction_id);
        }
        {
            PrecisionTimer t("TFIDRanker.Rank.GetImageCount");
            this->_number_of_documents = this->_image_storage->GetImageCount(reconstruction_id);
        }

        {
            PrecisionTimer t("TFIDRanker.Rank.RankAllSimilar");
            SIFT_Descriptor_weight_map query_weight_map = ComputeTFID(this->_query_word_frequencies);
            std::unordered_map<std::string, float> scores;
            for (auto id : similar)
            {
                auto map = this->_word_storage->GetAllDescriptors(reconstruction_id, id);
                double score = 0.0;
                if (details)
                {
                    ClosestNDetails_ClosestNImageDetails image_details;
                    score = SIFT_Descriptor_weight_map_norm_dot(query_weight_map, ComputeTFID(map, &image_details));
                    image_details.set_score(score);
                    (*details->mutable_image_details())[id] = image_details;
                }
                else
                {
                    score = SIFT_Descriptor_weight_map_norm_dot(query_weight_map, ComputeTFID(map));
                }
                scores[id] = score;
            }
...
private:
    SIFT_Descriptor_weight_map ComputeTFID(SIFT_Descriptor_count_map &in_map, ClosestNDetails_ClosestNImageDetails *details = nullptr)
    {
        ...
        for (auto e : in_map)
        {
            ...
            unsigned int word_frequency = this->_global_word_frequencies[word];
            ...
            if (details)
            {
                //std::string word_str((char *)word.data(), SIFT_Descriptor::static_size);
                //(*details->mutable_descriptor_scores())[std::to_string(std::hash<std::string>{}(word_str))] = score;
            }
        }
        return out_map;
    }
